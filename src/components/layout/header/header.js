...
export const Header = ({path}) => {
    ...
    return (
        ...
        <div className={style.menu}>
            <nav className={'nav'}>
                <ul className={'navList'}>
                    {Object.entries(translatedNavigation).map((item, i) => {
                        const data = item[1];
                        return <NavItem   {...data} currentLng={currentLng} onClose={onClose}
                                          active={path === data.link.replace(/\//g, '')} key={i} id={i}/>
                    })}
                </ul>
            ...
            <StaticImage className={'menuImg  '} objectFit={'contain'} width={500} placeholder={'tracedSVG'}
                         src={"./menuImg.jpg"} alt={""}/>
        </div>
